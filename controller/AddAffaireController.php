	...
	if(!empty($_POST['client'])){
		...
		if($statut=="archivee"){
			if(!file_exists('../affaires/affaire'.$idAffaire)){
				mkdir("../affaires/affaire".$idAffaire, 0777, true);
				chmod("../affaires", 0777);
				chmod("../affaires/affaire".$idAffaire, 0777);	
			}
			...
			for($i=0;$i<$numberOfFilesUploaded;$i++){
				...
				move_uploaded_file($_FILES['documentsAffaire']['tmp_name'][$i], '../affaires/affaire'.$idAffaire.'/'.$nameUpload);
			}
			...
			<?php
			    ...
			    try{
					...
					$document = "c://wamp/www/TopEntreprise/affaires/affaire".$idAffaire."/rapport-affaire".date('Y-m-d-h-i').".pdf";
		 ...
		 * ############################################
		 * */
		$_SESSION['affaire-add-success']="Affaire ajoutée avec succès.";
		header('Location:../add-affaire.php');	
	}
	else{
		$_SESSION['affaire-add-error']="<strong>Erreur Affaire</strong> : Vous devez remplir 'Nom Client' et 'Date de sortie'.";
		header('Location:../add-affaire.php');
	}
