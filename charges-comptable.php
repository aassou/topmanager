...
//classes loading begin
    ...
    if(isset($_SESSION['user'])){
		...
		$comptables = $comptableManager->getComptableByIdCharge($idCharge);
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a>Gérer les charges du comptable</a>
							</li>
						</ul>
				...
				<div class="tab-pane active" id="tab_1">
                   <div class="portlet box green">
                      <div class="portlet-title">
                         <h4><i class="icon-edit"></i>Ajouter charges du comptable</h4>
                         <div class="tools">
                      ...
                      <div class="portlet-body form">
                         <!-- BEGIN FORM-->
                         <?php if(isset($_SESSION['charges-comptable-success'])){ ?>
                         	<div class="alert alert-success">
								<button class="close" data-dismiss="alert"></button>
								<?= $_SESSION['charges-comptable-success'] ?>		
							</div>
                         <?php } 
                         	unset($_SESSION['charges-comptable-success']);
                         ?>
                         <?php if(isset($_SESSION['charges-comptable-error'])){ ?>
                         	<div class="alert alert-error">
								<button class="close" data-dismiss="alert"></button>
								<?= $_SESSION['charges-comptable-error'] ?>		
							</div>
                         <?php } 
                         	unset($_SESSION['charges-comptable-error']);
                         ?>
                         <form action="controller/AddChargesComptableController.php" method="POST" class="horizontal-form">
                            <div class="row-fluid">
                               <div class="span4 ">
						...
						<table class="table table-hover">
							...
							<tbody>
								...
								foreach($comptables as $comptable){
									$deletePath = "controller/DeleteChargesComptableController.php?idComptable=".$comptable->id();
								?>
								<tr>
									<td><?= $comptable->operation() ?></td>
									<td><?= $comptable->montant() ?></td>
									<td><?= date('d-m-Y | h:i:s',strtotime($comptable->dateOperation())) ?></td>
									<td><a href="charges-comptable-update.php?idComptable=<?= $comptable->id() ?>">Modifier</a></td>
									<td>
										<a href="#delete<?php echo $comptable->id();?>" data-toggle="modal" data-id="<?php echo $comptable->id(); ?>">
											Supprimer				
										</a>
									</td>
								</tr>
								<!-- delete box begin-->
									<div id="delete<?php echo $comptable->id();?>" class="modal hide fade in" tabindex="-1" role="dialog" aria-labelledby="login" aria-hidden="false" >
										<div class="modal-header">
											<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
											<h3>Supprimez les charges du comptable</h3>
										</div>
										<div class="modal-body">
											<form class="form-horizontal loginFrm" action="<?php echo $deletePath;?>" method="post">
												<p>Êtes-vous sûr de vouloir supprimer les charges comptable ?</p>
												<div class="control-group">
													<label class="right-label"></label>
													<input type="hidden" name="idCharges" value="<?= $comptable->idCharge() ?>" />
